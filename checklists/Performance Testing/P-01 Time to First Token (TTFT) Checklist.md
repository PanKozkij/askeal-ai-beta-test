...
- [ ] Use a simple prompt (e.g., “Hi, how can I test an AI chatbot?”)
- [ ] Measure time from **Send click → first token appears**
- [ ] Repeat 3 times in the same session
- [ ] TTFT values are stable (no extreme outliers)
...
- [ ] Measure TTFT 3 times across separate sessions
- [ ] TTFT stays within an acceptable range between sessions

---

### 3. Medium Prompt (2–3 Sentences)

- [ ] Use a prompt with ~2–3 sentences (typical user question)
- [ ] Measure TTFT 3 times in one session
- [ ] Compare TTFT vs. short prompt baseline
- [ ] No major degradation just because the prompt is slightly longer

---

### 4. Long Prompt (Several Paragraphs)

- [ ] Paste a longer prompt (multiple paragraphs, e.g., detailed QA scenario)
- [ ] Measure TTFT 3 times
- [ ] TTFT may increase slightly but remains **within acceptable limits**
- [ ] No extreme delays or UI “freeze” before first token
...
- [ ] Then send a short prompt and measure TTFT
- [ ] Compare TTFT before and after long conversation
- [ ] No significant performance drop due to long context

---

### 6. Multiple Tabs / Sessions Open

- [ ] Open 2–3 tabs with Askeal simultaneously
- [ ] In each tab, send the same short prompt and measure TTFT
- [ ] TTFT does not spike dramatically in any tab
- [ ] No tab remains “stuck” with no first token

---

### 7. Slow Network Simulation (If Possible)

- [ ] Throttle network in DevTools (e.g., “Slow 3G” profile) if you can
- [ ] Measure TTFT for a short prompt
- [ ] TTFT increases but remains understandable for a slow network
- [ ] No broken behavior (e.g., no response at all while connection active)

---

### 8. Error / Timeout Handling

- [ ] Trigger conditions where backend is slow (e.g., known busy time, or intentionally long prompt)
- [ ] If TTFT exceeds a reasonable threshold, UI shows spinner / feedback
- [ ] In case of complete failure, user sees a **clear error** instead of endless waiting
- [ ] No raw error messages or stack traces are exposed
